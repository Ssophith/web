...
import "../component/JobForm.js";

export class MiniiZar extends HTMLElement {
    ...
    this.innerHTML = `
      <style>
        ...
        .add-news {
          ...
          padding: 5px;
          color: var(--color6);
          box-shadow: 0 4px 6px var(--color6);
        ...
        .add-news:hover {
          color: var(--color1);
          box-shadow: 0 8px 12px var(--color1);
      ...
      <div class="container">
        ...
        <main class="main">
          <div>
            <section>
              <h2 class="section-title">Идэвхтэй</h2>
              <div class="grid-layout" id="active-jobs">
                <div class="empty-message">Ачааллаж байна...</div>
              </div>
            </section>
            <section>
              <h2 class="section-title">Идэвхгүй</h2>
              <div class="grid-layout" id="inactive-jobs">
                <div class="empty-message">Ачааллаж байна...</div>
              </div>
            </section>
          </div>
          <button class="add-news" id="add-job-btn">
            <svg width="60" height="60" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 14 14">
              <path
                fill="#ffffff"
                stroke="currentColor"
                stroke-linecap="round"
                stroke-linejoin="round"
                d="M1.5 2.5a1 1 0 0 0-1 1v9a1 1 0 0 0 1 1h11a1 1 0 0 0 1-1v-9a1 1 0 0 0-1-1h-2m-7-2v4m7-4v4m-7-2h5m1 5.5h-5M7 5.5v5"
              />
            </svg>
          </button>
        </main>
      ...
      <div class="job-form-container" id="job-form-container">
        <div class="job-form-wrapper">
          <button class="close-form-btn" id="close-form-btn">&times;</button>
          <job-form></job-form>
        </div>
      </div>
    `;
  ...
  initEvents() {
    const addBtn = this.querySelector("#add-job-btn");
    const closeBtn = this.querySelector("#close-form-btn");
    const formContainer = this.querySelector("#job-form-container");
    ...
    // JobForm-оос зар нэмэгдсэн event сонсох
    document.addEventListener("job-added", () => {
      formContainer.classList.remove("show");
      this.loadJobs();
    });
  }
  ...
  async loadJobs() {
    ...
    if (!userId) {
      this.querySelector("#active-jobs").innerHTML = `
        <div class="empty-message">Нэвтэрнэ үү. Заруудыг харахын тулд нэвтэрсэн байх шаардлагатай.</div>
      `;
      this.querySelector("#inactive-jobs").innerHTML = `
    ...
    try {
      ...
      // Идэвхтэй зарууд (workDate ирээдүйд байгаа эсвэл тодорхойгүй)
      const activeJobs = jobs.filter(job => {
        if (!job.workDate) return true;
      ...
      // Идэвхгүй зарууд (workDate өнгөрсөн)
      ...
      this.renderJobs(activeJobs, "active-jobs");
      this.renderJobs(inactiveJobs, "inactive-jobs");
    } catch (error) {
      console.error("Заруудыг авахад алдаа:", error);
      this.querySelector("#active-jobs").innerHTML = `
  ...
  renderJobs(jobs, containerId) {
    ...
    if (jobs.length === 0) {
      container.innerHTML = `
        <div class="empty-message">Зар байхгүй байна</div>
      `;
      return;
    }

    const isInactive = containerId === "inactive-jobs";
...
customElements.define("minii-zar", MiniiZar);
